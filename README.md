...
The SDLC_IDE project is guided by a set of key architectural principles:

*   **Human-Governed Core:** The Core DAG is immutable to autonomous agents. Core modifications require human approval via the ADR process, ensuring stability.
*   **Agent-Driven Extensibility:** The Mesh Layer allows agents to register new document types and relationships, enabling flexible, adaptable workflows.
*   **Single Source of Authority (SSoA):** The Core DAG is the definitive source of truth for the SDLC lifecycle. The Mesh can enrich this data, but never override it.
*   **Clear Governance Boundaries:** The Orchestrator and Governor components enforce a strict separation of concerns between the Core and the Mesh, as defined in ADR-004.
...
ADRs are subject to a strict approval process, enforced by an OPA policy, which requires that they be submitted via a pull request and reviewed by the project maintainers. Direct commits to `main` are disallowed for ADR changes.
...
The following command will execute the OPA policy against all ADRs in the `docs/architecture/design` directory:
...
This section provides a high-level overview of the repository's structure to help you navigate the codebase.
...
This section provides a few practical examples to illustrate the concepts discussed in this document.

### Example: PRD → TSD Workflow

1.  **A new PRD is created:** A new `prd-001.md` file is created in the `docs/prd` directory.
2.  **The Orchestrator detects the new file:** The Orchestrator detects the new file and creates a new node in the Core DAG.
3.  **A TSD is created:** A new `tsd-001.md` file is created in the `docs/tsd` directory, with a reference to `prd-001.md`.
4.  **The Orchestrator links the TSD to the PRD:** The Orchestrator detects the new file and links it to the PRD node in the Core DAG.

### Example: Mesh Extension File

A Mesh extension file is a simple YAML file that defines a new document type and its relationships. For example, the following file defines a new "Compliance Report" document type:
...
### Example: Agent Event Emission

An agent can emit an event to the Event-Based Observer Layer by sending a simple JSON payload to the event bus. For example, the following payload could be used to report a new compliance violation:

```json
{
  "event_type": "compliance_violation",
  "document_id": "tsd-001",
  "message": "The TSD does not comply with the new security policy."
}
```
