...
Embeddings will be used to enable drift detection, but the decision logic will reside in the Orchestrator (ADR-004) and specialized agents (e.g., `ContradictionDetector`).
...
- Pass orchestrator validation (as defined in ADR-004).

Further details on this process are available in ADR-005.

### 6. Embedding Storage

As defined in ADR-008, embeddings are split into version-controlled manifests and non-versioned vectors.
