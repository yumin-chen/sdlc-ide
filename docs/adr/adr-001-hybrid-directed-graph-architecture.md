...
> **The Orchestrator MUST reject any Core mutation whose provenance is not a human-approved ADR.**
...
* **Single Source of Authority (SSoA) Principle:** The Core DAG is the single authoritative source of SDLC lifecycle truth; the Mesh enriches but never overrides it.
* **Edge Directionality:** Mesh → Core edges imply read-only access; Core → Mesh edges imply non-authoritative references.
* Orchestrator validates structural rules.
* Governor enforces ACL, policy, and compliance on **both Core and Mesh, with stricter policies applied to Core artifacts.**
...
* Embeddings MAY propose semantic links; the Orchestrator MUST validate and MAY reject them. Embeddings NEVER imply structural authority.
* The Orchestrator always has final authority over lifecycle transitions.
