...
When a new version of an embedding model is released, a reindexing strategy will be triggered. This will involve re-embedding all affected artifacts and updating the vector database. The process will be managed through the embedding manifests defined in ADR-004. During this time, the system will continue to function using the old embeddings, but will gradually switch over to the new embeddings as they become available.
