---
title: Hybrid DAG & Mesh Extension Architecture
status: Accepted
date: 2025-11-27
...
# ADR-001: Hybrid DAG & Mesh Extension Architecture
...
graph TD
    ...
    Orch -- Validates ACL --> API
    Orch -- Validates ACL --> Perf
...
*Note: The specific technologies for the Event-Based Observer Layer (ADR-002), Semantic Inference (ADR-003), Orchestrator (ADR-004), and Custom Document Type Integration (ADR-005) will be detailed in separate, forthcoming ADRs.*
...
### Negative
- **Increased Complexity:** The hybrid model is inherently more complex to manage than a pure DAG or mesh architecture.
- **Orchestrator Bottleneck:** If not designed for high availability and low latency, the Orchestrator could become a single point of failure and a performance bottleneck.
- **Schema Management Overhead:** Mesh nodes require explicit schema and ACL definitions, which introduces management overhead and a learning curve for teams. This will be further detailed in ADR-005.
- **Validation Logic:** The system requires robust validation logic to prevent accidental cycles and ensure compliance with architectural constraints.
