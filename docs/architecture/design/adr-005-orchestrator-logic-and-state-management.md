...
To support high-frequency Mesh updates, we cannot re-traverse the entire graph on every edit. We will use a **localized traversal** strategy, which provides a pragmatic balance between performance and implementation complexity.

**Correct Logic:**
Let $G = (V, E)$ be the graph. When a request arrives to add a new edge $A \to B$:

1.  **Self-Loop Check:** Immediately reject if $A = B$. A node cannot depend on itself.

2.  **Localized Forward Traversal:** To check if adding the edge $A \to B$ would create a cycle, we must verify that a path from $B$ to $A$ does not already exist. Perform a graph traversal (DFS or BFS) starting from node $B$, following outbound edges.
    *   **If `A` is visited:** A path $B \to \dots \to A$ already exists. Adding the edge $A \to B$ would close this path into a cycle. The request is **rejected**.
    *   **If the traversal completes without visiting `A`:** No path from $B$ to $A$ exists. The edge is safe to add.

3.  **Core vs. Mesh Validation:**
    *   If both $A$ and $B$ are in the Core DAG, ensure the edge conforms to the strict `PRD -> TSD -> ADR -> KB` flow.
    *   If the edge involves a Mesh node, ensure it is declared in the source node's `allowed_outbound` schema.

**Rationale for this Approach:**

*   **Performance:** For a graph of our expected size ($|V| < 10,000$, $|E| < 50,000$), a localized DFS/BFS is extremely fast (typically milliseconds). It avoids the overhead of a full graph traversal and is sufficient for our real-time validation needs.
*   **Simplicity:** This method is well-understood and simpler to implement and debug compared to more complex online cycle detection algorithms.
*   **Sufficiency:** Since edge additions are transactional and validated one by one by the Orchestrator, we do not need a more complex algorithm that can handle simultaneous, un-validated edge insertions.

**Alternatives Considered:**

*   **Pearce's Algorithm:** An advanced incremental algorithm that can be faster for very high-frequency edge additions and deletions. We decided against it because its implementation complexity is significantly higher, and our performance bottleneck is more likely to be in agent processing or I/O than in this validation step.
*   **Global Topological Sort:** Maintaining a global topological sort of the graph and checking for consistency on each edit. This is too slow for real-time validation, as it requires processing a significant portion of the graph on every change.
