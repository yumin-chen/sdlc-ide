...
The Mesh supports user-defined document types, lateral workflows, and semantic relationships *without affecting the Core*. Custom workflows operate as independent DAGs within the Mesh Extension Layer and do not modify or extend the Core SDLC DAG.
...
Will define the core lifecycle state machine, cycle detection, mesh validation, transaction boundaries, and Governor integration.

### ADR-004 Persistence

Will define how the workspace separates core and extension artifacts, supports versioning, and enforces transactional writes.

### ADR-006 Extensions

Will define the process for declaring custom type schemas, edges, and embedding strategies, and how they pass validation.
...
## 12. Appendix A — Custom Workflow Registration & Enforcement

### Workflow Registration File

*   **Location:** `.sdlc_ide/workflows/<workflow-id>.yaml`
*   **Required fields:** `id`, `version`, `author`, `status`, `scope`, `nodes`, `edges`
...
*   Permission / ACL enforcement (via OPA)
*   Backward compatibility (no invalid artifact states; migrations must be explicit)

### Activation Lifecycle

1.  Contributor submits PR with workflow file.
2.  CI validates structure, semantics, and OPA rules.
3.  Orchestrator performs final validation on merge.
4.  Workflow status set to `active`.
5.  Activation event written to Event Stream.

---
## 13. Appendix B — Example State Transition
