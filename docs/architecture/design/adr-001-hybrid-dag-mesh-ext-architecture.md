...
* Supports user-defined document types and custom workflows safely. Custom workflows operate as independent DAGs within the Mesh Extension Layer and do not modify or extend the Core SDLC DAG.
...
* **ADR-005 (Orchestrator):** Defines the core state machine, cycle detection, and validation logic.
* **ADR-004 (Persistence):** Defines how artifacts are stored and how the DAG is enforced.
* **ADR-006 (Custom Integration):** Defines the process for declaring and validating custom document types.
